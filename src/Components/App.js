...
function App() {
  ...
  // all the data; might have them in individual components:
  const [contact, setContact] = useState(
    [
      {
        organization: "Doggo Rescues Peeps",
  ...
  // ====================== Fetch call (could also use axios) to pull from an API ===========================
  useEffect(() => {
  ...
  const meetMe = (dog) => {
    setCurrentDog(dog)
    // push the current value of setCurrentDog to the /meetme page:
    history.push("/meetme")
  };
